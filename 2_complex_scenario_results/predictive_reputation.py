...
class PredictiveReputationEngine:
    ...
    def get_early_warning_score(self, vehicle_id: str) -> float:
        ...
        recent_deviations = [r.deviation for r in list(history)[-5:]]
        recent_anomalies = sum(1 for r in list(history)[-5:] if r.is_anomalous)
        
        avg_deviation = np.mean(recent_deviations)
        anomaly_rate = recent_anomalies / min(5, len(history))
        
        warning_score = 0.6 * avg_deviation + 0.4 * anomaly_rate

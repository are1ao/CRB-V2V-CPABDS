...
class LocalMisbehaviorDetectionModule:
    ...
    def __init__(self, local_threshold: float = 0.25):
        self.local_threshold = local_threshold
        self._pending_reports: List[LMDMReport] = []

    def detect(self, vehicle_id: str, observation: Dict) -> LMDMReport:
        pos_err = min(observation.get("position_error", 0.0) / 1.0, 1.0)
        vel_err = min(observation.get("velocity_error", 0.0) / 1.0, 1.0)
        ts_err = min(observation.get("timestamp_error", 0.0) / 0.5, 1.0)
        freq = observation.get("message_frequency", 10.0)
        freq_err = min(abs(freq - 10.0) / 5.0, 1.0)
